...
<head>
    ...
    <style>
    #Branoils tr:nth-child(even){background-color: #b2b2b2;}
    </style>
    @livewireStyles
</head>
<body>
    ...
    @livewireScripts
    <script>
        window.addEventListener('OpenAddBranoilModal', function(){
            $('.addCountry').find('span').html('');
            $('.addCountry').find('form')[0].reset();
            $('.addCountry').modal('show');
        });
        window.addEventListener('CloseAddBranoilModal', function(){
            $('.addCountry').find('span').html('');
            $('.addCountry').find('form')[0].reset();
        ...
        window.addEventListener('swal:deleteBranoils', function(event){
            swal.fire({
                title:event.detail.title,
                html:event.detail.html,
                icon: 'warning',
                ...
                if(result.value){
                    window.livewire.emit('deleteCheckedBranoils',event.detail.checkedIDs);
                }
            });
        });
    </script>
...
<script>
    ...
    {
        var data = document.getElementById('Branoils');
        var file = XLSX.utils.table_to_book(data, {sheet: "AML Bran Oil Unit"});
        XLSX.write(file, { bookType: type, bookSST: true, type: 'base64' });
        XLSX.writeFile(file, 'AML Bran Oil Unit.' + type);
    }
