...
<body>
    ...
    @livewireScripts
    <script>
           window.addEventListener('OpenAddcusconModal', function(){
                $('.addCountry').find('span').html('');
                $('.addCountry').find('form')[0].reset();
                $('.addCountry').modal('show');
           });
           window.addEventListener('CloseAddcusconModal', function(){
               $('.addCountry').find('span').html('');
               $('.addCountry').find('form')[0].reset();
           ...
           window.addEventListener('swal:deletecuscons', function(event){
               swal.fire({
                   title:event.detail.title,
                   html:event.detail.html,
                   icon: 'warning',
                   ...
                   if(result.value){
                       window.livewire.emit('deleteCheckedcuscons',event.detail.checkedIDs);
                   }
               });
           });
    </script>
...
<script>
    ...
    {
        var data = document.getElementById('cuscons');

        var file = XLSX.utils.table_to_book(data, {sheet: "IT Custudy Construction"});

        XLSX.write(file, { bookType: type, bookSST: true, type: 'base64' });

        XLSX.writeFile(file, 'IT Custudy Construction.' + type);
    }
