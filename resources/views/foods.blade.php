...
<head>
  ...
  <style>
  #foods tr:nth-child(even){background-color: #b2b2b2;}
  </style>
  @livewireStyles
</head>
<body>
  ...
  @livewireScripts
  <script>
    window.addEventListener('OpenAddFoodModal', function(){
      $('.addCountry').find('span').html('');
      $('.addCountry').find('form')[0].reset();
      $('.addCountry').modal('show');
    });
    window.addEventListener('CloseAddFoodModal', function(){
      $('.addCountry').find('span').html('');
      $('.addCountry').find('form')[0].reset();
    ...
    window.addEventListener('swal:deleteFoods', function(event){
      swal.fire({
        title:event.detail.title,
        html:event.detail.html,
        icon: 'warning',
        ...
        if(result.value){
          window.livewire.emit('deleteCheckedFoods',event.detail.checkedIDs);
        }
      });
    });
    $.ajaxSetup({
    ...
    function html_table_to_excel(type)
    {
      ...
      var file = XLSX.utils.table_to_book(data, {sheet: "Igloo Foods Unit"});
      XLSX.write(file, { bookType: type, bookSST: true, type: 'base64' });
      XLSX.writeFile(file, 'Igloo Foods Unit.' + type);
    }
