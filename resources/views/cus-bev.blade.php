...
@livewireScripts
<script>
 window.addEventListener('OpenAddCusBeveModal', function(){
    $('.addCountry').find('span').html('');
    $('.addCountry').find('form')[0].reset();
    $('.addCountry').modal('show');
});
 window.addEventListener('CloseAddCusBeveModal', function(){
     $('.addCountry').find('span').html('');
     $('.addCountry').find('form')[0].reset();
 ...
 window.addEventListener('swal:deleteCusBeves', function(event){
     swal.fire({
         title:event.detail.title,
         html:event.detail.html,
         icon: 'warning',
         ...
         if(result.value){
             window.livewire.emit('deleteCheckedCusBeves',event.detail.checkedIDs);
         }
     });
 });
</script>
...
<script>
    ...
    {
        var data = document.getElementById('CusBeves');
        var file = XLSX.utils.table_to_book(data, {sheet: "IT Custudy Bev,Amsrl,Bocl"});
        XLSX.write(file, { bookType: type, bookSST: true, type: 'base64' });
        XLSX.writeFile(file, 'IT Custudy Bev,Amsrl,Bocl.' + type);
    }
