...
<body>
    ...
    @livewireScripts
    <script>
           window.addEventListener('OpenAddcusIglooModal', function(){
                $('.addCountry').find('span').html('');
                $('.addCountry').find('form')[0].reset();
                $('.addCountry').modal('show');
           });
           window.addEventListener('CloseAddcusIglooModal', function(){
               $('.addCountry').find('span').html('');
               $('.addCountry').find('form')[0].reset();
           ...
           window.addEventListener('swal:deletecusIgloos', function(event){
               swal.fire({
                   title:event.detail.title,
                   html:event.detail.html,
                   icon: 'warning',
                   ...
                   if(result.value){
                       window.livewire.emit('deleteCheckedcusIgloos',event.detail.checkedIDs);
                   }
               });
           });
    </script>
...
<script>
    ...
    {
        var data = document.getElementById('cusIgloos');

        var file = XLSX.utils.table_to_book(data, {sheet: "IT Custudy Igloo"});

        XLSX.write(file, { bookType: type, bookSST: true, type: 'base64' });

        XLSX.writeFile(file, 'IT Custudy Igloo.' + type);
    }
